...
device. 
He says that it has many fancy features, but the most important one to 
set up right now is the communication system. However, because he's heard you have significant experience dealing with 
...
"""
__version__ = '0.1.0'
__autor__ = "Regina Celi da Silva"

import os

file_txt = "input-day6.txt"
FILEPATH = os.path.join(os.curdir, file_txt)

with open(FILEPATH) as file:
    input = file.read()


def processed_marker(char_number):
    for i in range(char_number, len(input)):
        group = set(input[(i-char_number):i])
        #print(group)
        if len(group)== char_number:
            print(f"Marker for {char_number} char number : {i}")
            break
